...
/**
 * The <code>AliasTypeCode</code> class represents a <code>TypeCode</code>
 * object which is associated with an IDL typedef.
 * 
 * @autor Juan A. C&aacute;ceres
 * @version 1.0
 */

public class AliasTypeCode extends ComplexTypeCode
{
    ...
    public AliasTypeCode(String repositoryId, String name, TypeCode type)
    {
        super(TCKind.tk_alias, repositoryId, name);
        _type = type;
    }
    ...
    public boolean equal(org.omg.CORBA.TypeCode tc)
    {
        ...
        if (!super.equal(tc))
            return false;
        try {
            return _type.equal(tc.content_type());
        }
     ...
     * @pre the the <code>TypeCode</code> must be an alias type
     */

    public static void write_params(TypeCode type, CDROutputStream output)
    {
        try {
    ...
    {
        try {
            return TypeCodeMarshaler.valuesEqual(type.content_type(),
                                                 input_a,
                                                 input_b);
        }
        catch (BadKind bk) {
